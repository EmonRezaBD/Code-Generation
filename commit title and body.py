...
# Function to scrape the commit title and body (with code) from GitHub
def scrape_github_commit(url):
    ...
    # Extract the commit body, including the code changes
    commit_body_element = soup.find("div", class_="js-file-line-container")
    commit_body = commit_body_element.get_text('\n', strip=True) if commit_body_element else "No Body Found"
    
    # Return a dictionary with the commit title and body
    return {
...
def main():
    ...
    for index, row in df.iterrows():
        ...
        # Scrape the commit title and body (with code) from the GitHub commit page
